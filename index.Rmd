...
Testeo el mapa hielo
```{r ssmugri}
gl <- ggplot() +
   geom_sf(data=grilen %>% 
             drop_na(length_total_cm), 
           aes(fill = length_total_cm), 
           alpha=0.7, color=NA) +
...
join data intro `strata`object

```{r}
ohbio7 <- ohbio6 %>% 
  dplyr::group_by(Year, geometry, gear_type) %>% 
  dplyr::summarise(meanl=mean(length_total_cm))



# comprobar si hay geometrías no válidas
invalidos <- st_is_valid(ohbio7)
...
datae <- load("~/DOCAS/Data/KrillLows/DataEnvKrill2.RData")
