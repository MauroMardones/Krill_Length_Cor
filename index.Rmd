...
population dynamics of krill (*Euphausia superba*).
...
Filter data regarding to previous glances. Follow with a quick glimse to
...
In this step we filter the empty or null data (`NA`) and we will nest
the mean lengths in the same formats as the environmental data.
...
Identificar tendencias temporales por cada SSSMU para tener un mejor
detalle del comportamiento de la variable a traves del tiempo en el
espacio definido.
...
corPlot(envlen4, cex = 1.2, main = "Matriz de correlación")
ma <- cor(envlen4)
