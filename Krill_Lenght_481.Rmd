...
Firsts glance. Test how many register have by year. In this case, `length_total_cm` by year. 
...
ohpor <- ohbio2 %>% 
  ...
  mutate(infre=(length_total_cm - 3.6)) %>%
  filter(infre > 0) %>% 
  mutate(prc =round(infre/length_total_cm*100,2))
```
Ahora lo grafico con un polar plot por subarea

```{r message=F, warning=F}

ohbiopor<- ggplot(ohpor) + 
  geom_point(aes(y=prc, 
                 x=season_ccamlr))+
  facet_wrap(~asd_code, ncol=3)+
  theme_bw()+
...
```{r}
box <- ggplot(ohpor) +
  geom_boxplot(aes(prc, group=season_ccamlr), 
               alpha=0.5,
               fill=3)+
  xlim(0, 75)+
  coord_flip()+
