...
Test how many register have by year. In this case, `length_total_cm` by year.
...
```
a quick glimse to all 48 subarea length composition from monitoring fisheries.
...
```{r fig.align="center", message=F, warning=F}
jn <- ggplot(ohbio2 , aes(x=length_total_cm, 
                          y = as.factor(season_ccamlr),
                         fill=asd_code))+
  geom_density_ridges(alpha=0.5, stat = "binline") + 
  facet_wrap(.~vessel_nationality_code, ncol=8) +   
  geom_vline(xintercept = 3.6, color = "red")+
  scale_x_continuous(breaks = seq(from = 1, to = 10, by = 1))+
  scale_y_discrete(breaks = seq(from = 2001, to = 2020, by = 1))+
  scale_fill_viridis_d(option = "F", name="Nacionalidad")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  xlim(0,10)+
  xlab("Longitud (cm.)")+
...
```{r}
ohbpor= ohbio2 %>% 
  dplyr::mutate(inf_112=(length_total_cm - 3.6))%>% # new column TRUE if value is below 112
  dplyr::summarise(pct=as.double(round(100*sum(inf_112))/n())) # divide the number or rows where value is below 112 by the total number of rows
```
